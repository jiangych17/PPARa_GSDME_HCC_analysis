...
kegg_geneset_file <- "KEGG_mouse.xls"   # each sheet = one KEGG pathway; first column = mouse gene symbols
kegg_outdir <- file.path(outdir, "Mouse_KEGG_GSEA_local_fgsea")
dir.create(kegg_outdir, showWarnings = FALSE, recursive = TRUE)

if (!file.exists(kegg_geneset_file)) {
  stop("Local KEGG gene-set file not found: ", kegg_geneset_file)
}

kegg_sheet_names <- excel_sheets(kegg_geneset_file)
print(kegg_sheet_names)

kegg_pathways <- list()

for (s in kegg_sheet_names) {
  gs <- read_excel(kegg_geneset_file, sheet = s, col_names = FALSE)[[1]]
...
print(sapply(kegg_pathways, length))

kegg_overlap_info <- data.frame(
  pathway = names(kegg_pathways),
  geneset_size = sapply(kegg_pathways, length),
  overlap_with_matrix = sapply(kegg_pathways, function(x) sum(x %in% names(ranks)))
)

print(kegg_overlap_info)

write.csv(
  kegg_overlap_info,
  file.path(kegg_outdir, "Mouse_KEGG_GeneSet_overlap_with_matrix.csv"),
  row.names = FALSE
)
...
kegg_fgsea_res_export <- kegg_fgsea_res %>%
  mutate(leadingEdge = sapply(leadingEdge, paste, collapse = ";"))

fwrite(
  kegg_fgsea_res_export,
  file.path(kegg_outdir, "Mouse_KEGG_GSEA_local_fgsea_results.csv")
)

kegg_leading_edge <- kegg_fgsea_res_export %>%
  select(pathway, leadingEdge)

write.csv(
  kegg_leading_edge,
  file.path(kegg_outdir, "Mouse_KEGG_leading_edge_genes.csv"),
  row.names = FALSE
)

cat("Top KEGG GSEA results:\n")
print(head(kegg_fgsea_res_export, 20))

## =========================
## KEGG GSEA dotplot
## =========================

if (nrow(kegg_fgsea_res_export) > 0) {
  
  kegg_plot_df <- kegg_fgsea_res_export %>%
    arrange(padj) %>%
    slice_head(n = 20) %>%
    mutate(
  ...
  p_kegg_dot <- ggplot(
    ...
    labs(
      ...
      size = "-log10(FDR)",
      color = "NES",
      title = "Mouse KEGG GSEA",
      subtitle = "FILM vs Non-FILM; NES < 0 indicates suppression in FILM"
    )
  
  ggsave(
    file.path(kegg_outdir, "Mouse_KEGG_GSEA_dotplot_top20.pdf"),
    p_kegg_dot,
    width = 8,
    height = 6
  )
  ...
  ggsave(
    file.path(kegg_outdir, "Mouse_KEGG_GSEA_dotplot_top20.png"),
    p_kegg_dot,
    width = 8,
    height = 6,
    dpi = 300
...
fwrite(
fgsea_res_export,
file.path(outdir, "Mouse_focused_GO_cell_death_GSEA_results.csv")
)

leading_edge <- fgsea_res_export %>%
select(pathway, leadingEdge)

write.csv(
leading_edge,
file.path(outdir, "Mouse_leading_edge_genes.csv"),
row.names = FALSE
)
...
## =========================
## Single and combined enrichment plots
## =========================
...
## =========================
## Export mouse expression matrix for ImmunCellAI
## =========================
